  ...
  const handleDropdownSelectTwo = (vetId) => {
    ...
    function showSelectedVetHours(selectedItemTwo, stringDate, vetScheduleInfoTable, vetAppointmentInfoTable) {
        ...
        appointments.forEach(appointment => {
            ...
            for (let i = 0; i < arry.length; i++) {
                const fixingArry = arrayOfAvails[i].split(" ");
                let startHourAva = fixingArry[0];
                let startPeriodAva = fixingArry[1];
                let dashAva = fixingArry[2];
                let endHourAva = fixingArry[3];
                let endPeriodAva = fixingArry[4];
                if (startHourAva == 0) {
                    arrayOfAvails[i] = `${12} am ${dashAva} ${endHourAva + 1} am`;
                }
                if (startHourAva == 12) {
                    arrayOfAvails[i] = `${12} am ${dashAva} ${endHourAva + 1} am`;
                }
                arrayOfAvails[i] = `${startHourAva} ${startPeriodAva} ${dashAva} ${endHourAva + 1} ${endPeriodAva}`;
