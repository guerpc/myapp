...
import {
    ...
    DropdownMenu, DropdownItem, Modal, ModalHeader, ModalBody, ModalFooter, Container, Row, Col, Table
} from 'reactstrap';
...
const CalFour = (props) => {
  ...
  const handleDropdownSelectTwo = (vetId) => {
    ...
    function ShowPetRecordInfo(petRecordsInfoTableTarget, selectedItem ) {
        const petRecordsInfoTable = UsePetRecordsInfoTableTarget(petRecordsInfoTableTarget);
        let petiden = selectedItem.toString();
        if (!petRecordsInfoTable) {
            return <div>Loading...</div>;
        }
        return (
            <Container fluid>
                <Row>
                    <Col>
                        <Table>
                            <thead>
                                <tr>
                                    <th>PetRecordID {selectedItem.toString()}</th>
                                    <th>PetID</th>
                                    <th>PetMedsUsed</th>
                                    <th>PetNotes</th>
                                    <th>PetOutcome</th>
                            ...
                            <tbody>
                                {petRecordsInfoTable
                                    .filter((record) => record.PetID == selectedItem)
                                    .map((record) => (
                                        <tr key={record.PetRecordID}>
                                            <td>{record.PetID}</td>
                                            <td>{record.PetWeight}</td>
                                            <td>{record.PetMedsUsed}</td>
                                            <td>{record.PetNotes}</td>
                                            <td>{record.PetOutcome}</td>
                                            <td>{record.PetRecom}</td>
                                            <td>{record.PetRecordDate}</td>
                                        </tr>
    ...
    return (
        <div>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                <Button onClick={toggleShowPetInfo}>Show Pet Info</Button>
                { <ShowPetRecordInfo petInfoTableTarget={petInfoTableTarget} selectedItem={selectedItem} />}
